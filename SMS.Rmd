...
library(ggplot2)
ggplot(Distribution, aes(x = Label, fill = Percentage)) + theme_bw() + geom_bar() + labs(y = "Percentage Distribution", x = "Message Label", title = "Distribution of Message Labels")
```
```{r}
...
wordcloud(BagOfWords, max.words = 300, random.order = FALSE, scale=c(8,.3), colors = pal)
```
...
findFreqTerms(FinalData, lowfreq = 300)
```

```{r}

sparseWords <- removeSparseTerms(FinalData, 0.995)
...
#confusionMatrix(predict(SMS_GLM, newdata = test_data), test_data$MessageLabel)
